...
function BattleshipChallengeViewModel(canvas) {
    ...
    self.codeMirror.setValue("function yourScript() {\n  // Put everything in this function (inner functions allowed)" +
        ...
        "\n" +
        "  self.isSafeShot = function(x, y){\n" +
        "    return self.getBoard()[x][y].isHidden;\n" +
        "  };\n" +
        "\n" +
        "  self.sweepBoard = function(){\n" +
        "    for(var x = 0; x < self.getBoardHeight(); ++x){\n" +
        "      for(var y = 0; y < self.getBoardWidth(); ++y){\n" +
        "        if(self.isSafeShot(x,y)){\n" +
        "          return [x,y];\n" +
        "        }\n" +
        "      }\n" +
        "    }\n" +
        "  };\n" +
        "\n" +
        "  self.shootRandom = function(){\n" +
        "    while(true){\n" +
        "      var rand_x = Math.floor(Math.random() * self.getBoardWidth());\n" +
        "      var rand_y = Math.floor(Math.random() * self.getBoardWidth());\n" +
        "      if(self.isSafeShot(rand_x, rand_y)){\n" +
        "        return [rand_x, rand_y];\n" +
        "      }\n" +
        "    }\n" +
        "  };\n" +
        "\n" +
        "  //return self.shootRandom();\n" +
        "  return self.sweepBoard();\n" +
        "}\n");
    ...
    self.setCode = function () {
        ...
        if (!self.user()) {
            ...
            self.database.ref('users-open-house/' + self.user().uid).update({
    ...
    self.updateHighScore = function(score) {
        if(self.user()) {
            self.database.ref('users-open-house/' + self.user().uid).once('value').then(function(snapshot) {
